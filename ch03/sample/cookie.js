...
또한 expire를 따로 지정해주지 않으면 기본적으로 브라우저가 닫힐 때 쿠키도 같이 날아감

클라이언트가 쿠키와 함께 요청 전송 시 req.headers.cookie를 통해 쿠키 값에 접근 가능
req.headers.cookie에 저장된 값은 문자열로, 이를 자바스크립트에서 사용하려면 객체로 파싱 필요
*/